...
| `M` | ∇・(∇Az/(μ0 μr)) = −Jz | DC インダクタンス行列 [H/m]。**内部インダクタンス込み、μr 対応** |
...
| `frequency` | 実数 [Hz] | 0 | 誘電損 (tanδ) を評価する周波数。0 なら tanδ は無視 |
| `current` | 実数 [A] | 1 | 静磁場解析の励振電流 |
| `analysis` | `C` / `L` / `R` / `M` の組 | `C` | 実行する解析 |
| `tline` | `X` / `Y` / `Z` | なし | 伝送線路軸。指定すると単位長あたりの値で出力する (`L`, `M` は必須) |
...
- **表皮効果は未対応**。`Rs` は DC (電流一様) の導体抵抗、`M` の L も DC 値。
  高周波では Rs は √f で増え、L は外部値 (`L`) に近づくので、
  Z0・ε_eff の計算には `L` (TEM) を使っている。
- 誘電損は `tanδ` を単一周波数の等価導電率として扱う。分散性 (Debye/Lorentz)
  の周波数依存材料は未対応。
- 渦電流 (時間調和 A-φ 定式化)、非線形磁性体は未対応。
