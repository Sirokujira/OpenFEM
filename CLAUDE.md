# OpenFEM

準静的 FEM による回路パラメータ (R/L/C) 抽出ソルバー (C)。
OpenFDTD (FDTD 電磁界ソルバー) と同じ構成・入力書式を踏襲した別リポジトリ。
実装は CPU (OpenMP)。MPI は任意依存 (`WITH_MPI=ON`) で周波数掃引の点を
プロセスに分散する (CUDA は未対応 — GPU の無い環境では検証できないため)。

## ビルド / テスト

```bash
cmake -B build -DCMAKE_BUILD_TYPE=Release
cmake --build build -j"$(nproc)"

# 回帰 (microstrip): Z0 / eps_eff が変更前と一致すること
mkdir -p /tmp/smoke && cp data/sample/microstrip.ofe /tmp/smoke/ && cd /tmp/smoke
$OLDPWD/bin/ofe -n 2 microstrip.ofe && grep "normal end" ofe.log

# 検証 (解析解との比較: 平行平板 1%, 抵抗 1%, 同軸 5%)
# 相対パスは script 内で cd するので絶対パスで渡すこと
sh data/sample/rlc_check.sh "$PWD/bin/ofe" "$PWD/bin/ofe_post" /tmp/rlc-check
```

## 移植性の絶対規則 (OpenFDTD が Windows CI で踏んだもの)

- **C99 VLA 禁止** (MSVC C2057/C2466)。`malloc` + 明示インデックスの
  フラット配列を使う。
- **OpenMP のループ変数は `int`**。MSVC の OpenMP 2.0 は 64bit のループ変数を
  受け付けない。節点数が `INT_MAX` 未満であることは `setup()` で確認済みなので、
  節点ループは `const int n = (int)A->n;` として `int` で回す。
- **float\*/double\* の取り違え禁止**: 配列の実型と読み出しポインタ型の
  不一致は Windows で 0xC0000005 クラッシュ。本リポジトリは全て `double`。
- libm リンクは CMake の `MATH_LIB` 変数経由 (Windows には m.lib が無い)。
- MSVC フラグは CMakeLists の既存ブロックに従う
  (`/utf-8`, `_USE_MATH_DEFINES`, `/STACK:16777216`)。
- 数学定数は `PI` / `EPS0` / `MU0` / `C0` の既存マクロを使う。

## 数値まわりの規則

- 剛性行列は CRS (27 点ステンシル)。構造格子なので格納位置は
  `crs_offset()` で探索なしに計算する。パターンを変えるときは
  `crs_alloc()` と `crs_offset()` を必ず同時に直す。
- Dirichlet 条件は行列を書き換えず、`fix[]` マスクで恒等行として扱う
  (`crs_spmv`)。解いた後に**元の行列**から反作用 Σ(Kφ) を取って
  電荷・電流にするため、行列を破壊してはいけない。
- 要素行列は 2x2x2 Gauss 積分 (直方体要素に対して厳密)。材料係数は
  `assemble.c` の `material_coef()` に集約する (mode 0..3)。
- 静磁場 (`analysis = M`) は純 Neumann 系。正味電流が 0 なので可解だが特異なので、
  1 節点だけ固定して解く。L は相互エネルギー ∫J・A から取るので定数分に依らない。
  **右辺 b は L の計算にそのまま使うので、ソルバーに渡すコピー側だけ
  固定節点を 0 にすること**。
- 渦電流 (`analysis = F`) は複素対称系 K + jωM を COCG で解く。内積に共役を
  取らない双一次形式を使うこと (共役を取ると収束しない)。境界は自然境界条件
  (磁気壁) のままにする。**Dirichlet で切ると帰路電流が境界に逃げて
  Z_loop が壊れる**。基準導体は V'=0 に固定し、ポートのみ励振して
  Y (np x np) を作り、Z_loop = inv(Y) とする。
- 複素数は実部・虚部の配列を別に持つ (C99 complex は MSVC 互換性の都合で使わない)。
- 非線形磁性体 (`bh`) は Newton-Raphson。ν の逐次代入は飽和領域で発散するので
  使わない。残差とヤコビアンで ν の評価点 (Gauss 点) を揃えること。
- 収束判定は相対残差。`solver` キーの既定は `10000 100 1e-9`。
- 直接解法 (`direct = 1`) は実対称なら Cholesky、複素対称 (`F` / `A`) なら
  **LDLᵀ** (√ が取れないので Cholesky は不可)。ピボット選択はしない
  (対称性が崩れてスカイラインの外に充填が出る)。破綻しないと言えるのは
  `F` だけなので、**複素側は分解後に真の残差を計算してログに出す**。
  `A` はゲージを固定しないと特異なので `gauge = 1` を要求する
  (丸めで通ってしまうことがあるが、それは並べ替えとコンパイラ次第の運)。
- 分散材料 (`debye`/`lorentz`/`drude`/`colecole`) は時間因子 e^{jωt}、ε = ε' − jε''。
  `er` に ε'、`ei` に **+ε''** (損失が正) を足す。極を足すときはこの符号規約に従う。
  Cole-Cole は α = 0 で Debye に厳密一致するので、その恒等式を検証に使う。
  Havriliak-Negami (`havriliak`) は β=1 で Cole-Cole、α=0 で Cole-Davidson、
  両方で Debye に厳密一致する。**極限を持つモデルは極限そのものを恒等式にする**
  (閉形式を別に用意しなくてよく、しかも厳密)。
- 温度依存は σ の読み出し**手前で一度だけ**掛ける (`input_data.c` の末尾)。σ の
  読み出しは `Material[].sigma` (R/A/E) と `CondSigma[]` (Rs/F) の 2 系統あり、
  下流で個別に補正すると必ず漏れる。
- **εr / μr の温度依存だけは読み出し時 (`material_coef_pub`) に掛ける。**
  σ と同じく入力時に掛けると、`material_freq()` が分散材料の εr を周波数毎に
  展開し直して上書きするので周波数掃引で補正が消える。読み出し時なら累積しない。
  B-H 曲線は再計算されないので σ と同じ扱いでよい。
- **材料構造体にフィールドを足したら、`material` で材料を増やす経路の初期化も
  必ず直す。** realloc した領域は未初期化なので、片方だけ直すと「2 番目以降の
  材料だけ挙動が変わる」不具合になる (実際に踏んだ)。
- **周波数に依存する量は `material_freq()` に集約し、べき等にすること。**
  周波数掃引は各点でこれを呼び直す。「既に埋まっているかどうか」で分岐すると
  2 回目以降スキップされて値が凍結する (実測: 分散材料の C が最初の周波数で
  固まった)。「利用者が明示したかどうか」のフラグで分岐する。
- **材料キーを足したら「どの解析が読むか」を `input_data.c` の未使用キー警告に
  登録すること。** 読まない解析に指定されても値が 0 になるだけで黙って通るため、
  警告が唯一の防波堤になる。警告は誤検知も同じくらい有害なので、
  正しいケースで 1 件も出ないことを `rlc_check.sh` で必ず両方向に検証する。
- 3 次元渦電流 (`analysis = A`) は A (辺) と φ (節点) の連成。**節点側の式を
  jω で割ると複素対称**になるので COCG がそのまま使える。この形にしないと
  (1,2) = TG と (2,1) = jωGᵀT が食い違って対称でなくなる。
- A-φ 系は (A, φ) → (A + Gψ, φ − jωψ) で不変なので特異。右辺を Dirichlet の
  持ち上げ b = −A x_D で作れば b は必ず値域に入るので COCG は収束し、
  Z はゲージ不変なので**ゲージ固定しない方が速い** (実測で反復 1/6、Z は 7 桁一致)。
  A・φ 自体が要るときだけ `gauge = 1` (tree-cotree)。その場合 awall の辺を
  優先して木に入れないと境界条件と端子電圧が壊れる。
- **1 次 Nedelec 要素の誤差は要素の最大寸法で決まる** (場が変化する方向の
  刻みではない)。場が一様な方向を粗くすると誤差が出る (実測で 13%)。
  辺要素を使う解析では要素を等方的にとること。
- 2 次四面体 (10 節点) は**等パラメトリック**。積分は Duffy 変換 + 各方向
  3 点 Gauss-Legendre (27 点)。四面体の少点数則 (Keast 等) は次数を上げると
  負の重みが出るうえ、係数を暗記に頼ると検算できないので使わない。
  Duffy は 1 次元 Gauss の積で書けるので導出が閉じている。
- **辺要素 (`E` / `A`) に 2 次格子を渡さない。** Whitney 形状関数は 1 次要素の
  頂点に基づくので、辺上の中間節点がどの要素にも現れず節点ブロックが特異になる。
  `setup_unstruct()` で弾く。
- **辺要素は種別の混在を許す** (四面体 + 角柱 / 六面体 + 角柱)。要素ごとに
  辺数・局所辺表が違うので、要素 e の辺は `EdgeOff[e]` から引く
  (純粋な格子では `e * nedge` に一致するので既存の答えは変わらない)。
  ピラミッドだけは多項式の Nedelec 基底が無いので弾く。
  **接する面の形が揃う向きにしか割れない**: 六面体 + 角柱は界面が四角形面
  (x 方向に並べる)、角柱 + 四面体は三角形面 (z 方向に積む)。逆向きに割ると
  四角形面を三角形 2 枚が覆う非適合な界面になる。
- **非適合な界面と「裂けた」格子は、要素毎の量の和で書ける恒等式では
  絶対に検出できない** (3 つとも実測で確認):
  ・四角形面を三角形 2 枚が覆う非適合 → `elem_face_check()` (位相) だけが頼り。
    接線連続性の検査すら素通りする (節点集合が違うのでそもそも共有面にならない)。
  ・節点をマージし忘れた格子 → `node_merge_check()` (頂点の座標の重複) だけ。
    (a)〜(g) が全部機械精度で通り、連結成分が増えたことしか痕跡が残らない。
  ・種別ごとの添字の取り違え (`e - NTet - NHex` の書き忘れ) → 自己整合な別の
    格子を読むだけなので恒等式は全部通る。**(g) が数える「種別をまたぐ面の数」
    が 0 になること**でだけ落ちる (`rlc_check.sh` で assert する)。
- **場の出力の検証 (`vtkcheck.awk`) はセル型で見分ける。** 節点数だけでは角柱
  (6 節点) と 2 次三角形 (6 節点) が区別できない。六面体・角柱・ピラミッドの
  体積はソルバーと同じ求積 (∫det J) で出すこと — 四面体に割った和は等パラ
  メトリック写像の体積ではない。**awk の式を複数行に分けないこと** (文が改行で
  終わるので第 1 項だけになる。この誤りはピラミッド格子でしか落ちない)。
- **{a + b×r} の恒等式は接線連続性を検査できない。** その場は要素毎に厳密に
  補間されるので、隣の要素とトレースが食い違ってもエネルギーは要素積分の和と
  して合う。面をまたぐ跳びを直接見る検査 (`analysis = E` の (g)) が要る。
- **六面体・角柱の辺要素は参照基底の共変 Piola 変換** (W = J⁻ᵀŴ,
  ∇×W = Jĉ/det J) で組む。辺の向き規約 (小さい節点番号 → 大きい方) と
  符号は四面体と共通。**E の恒等式 {a+b×r} はアフィンな要素でだけ厳密**
  (平行六面体 / 平行移動の角柱。ソルバー側で検査して外れたらエラー)。
  剛体回転した格子を必ず対で回す — 軸平行の格子では J が対角になり、
  Piola の添字の取り違えが検出できない。awall の面は四角形にもなる
  (4 辺を固定。漏らすと壁辺 0 で非受動 R = −4e9、実測)。混在格子と
  ピラミッドの辺要素は未対応のまま弾く (ピラミッドは多項式基底が無い)。
- 断面 2 次元の Az 定式化では、面内の異方性 ν は **B に掛かる**。
  B = ∇×(Az ê_t) なので ∇Az の基底では面内 2 成分が入れ替わり非対角の符号が
  反転する (c_pp = ν_qq, c_qq = ν_pp, c_pq = −ν_pq)。`material_coef_pub()` の
  mode 4 に集約してある。等方性では一致するので等方性ケースでは検出できない。
- 曲がった格子では「頂点 4 個から作った四面体の体積」は内接多角形の体積であって
  要素の体積ではない (実測 −4.5%)。閉形式と比べるときにここを取り違えると、
  正しい実装が落ちる**偽陽性**になる (実際に踏んだ)。

- **2 次の六面体 (20/27 節点)・角柱 (15/18 節点) の局所節点の並びは gmsh の
  出力を分類した実測の表** (`HEX2_XI` / `PRISM2_T`/`PRISM2_Z`) を使う。暗記や
  文献の表は書き手と読み手が同じ誤解を共有し得る。**2 次特有の基底 (辺・面・
  体心) を検査できるのは 2 次場の恒等式 (`analysis = P`) と 1 次との誤差比
  (`coax_hex2` vs `coax_h1c`) だけ** — 中間節点の基底を 0 にして黙って 1 次に
  退行させても、体積・線形場の恒等式・長方形格子の C はすべて素通りする
  (実測)。電極面 (9/8 節点四角形・6 節点三角形) は中間節点まで固定し、
  1 次の境界面が混ざる格子は弾く (固定を外すと C が −90%、実測)。
  次数の混在と「2 次の六面体・角柱 + 他種別」も弾く。
- 非構造格子の **8 節点六面体は等パラメトリック** (2x2x2 Gauss、直方体で厳密)。
  ヤコビアンは 8 節点すべてから**Gauss 点毎に**作ること。**平行六面体では J が
  要素内で一定になるので、中心で 1 回だけ評価する手抜きが厳密に正しくなる** —
  直方体・剛体回転・同軸のどれでもこの誤りは検出できず、ゆがんだ格子でだけ
  落ちる (`nodal_test_hex`)。局所節点の並びは Gmsh / VTK と同じで、構造格子の
  ビット順とは違うので表を共有しないこと。四面体との混在は弾く。
- **角柱 (6 節点) は三角形 3 点則 x ζ 方向 2 点 Gauss の 6 点**。六面体と同じく
  ヤコビアンは積分点毎に作ること (上下の三角形が平行移動の関係だと J が ζ に
  依らなくなり、手抜きが検出できない)。
- **3 次元要素は「四面体 → 六面体 → 角柱 → ピラミッド」の連番で見る**
  (`elem3d_count` / `elem3d_kind` / `elem3d_nodes` / `elem3d_mat` / `elem3d_tag`)。
  種別を足すときはこの 5 つと `assemble_elem3d` / `crs_alloc_elem3d` /
  `fieldout.c` / `hdf5out.c` を同時に直す。**並び順は変えないこと** —
  純四面体格子で従来と完全一致する (既存サンプルの出力がバイト一致することで
  担保している)。
- **六面体と四面体は面を共有できない。** 四角形面のトレースは双 1 次、三角形面は
  1 次なので、接すると有限要素空間が H¹ に入らない (値は出るが誤答)。
  角柱とピラミッドは両方の面を持つのでどちらにも適合し、間に挟めばつながる。
  六面体 + 四面体はピラミッドが 1 つも無ければ `elem_finish()` で入力エラー。
  **四角形面を三角形 2 枚で覆った非適合の面は、節点が全部共有されるので
  線形場の恒等式では検出できない** (1 次の場は両側で厳密に表せる)。
  `elem_face_check()` が位相で弾く: 表裏 2 回現れない四角形面の 3 節点部分集合を
  対になっていない三角形面と照合する。
- **ピラミッド (5 節点) は立方体座標で組む。** ξ = u(1−ζ), η = v(1−ζ) と縮めると
  有理形状関数が多項式になり、積分は 2x2x2 Gauss (ζ は [0,1]) のままでよい —
  (1−ζ)² の錐の縮みは det J に自動的に入るので、特異な重みも専用の積分則も
  要らない。**体積はどんな 5 節点ピラミッドでも厳密** (det J は各方向 2 次)。
  剛性が厳密なのは**底面が平行四辺形のとき** (頂点の位置は任意) で、そこから
  外れた形の検証には `nodal_test_pyr` (ゆがんだ格子) が要る。J は (1−ζ)² の
  因子のため**どんな形でも要素内で変化する**ので、「中心で 1 回だけ評価する」
  手抜きを隠す特別な形は存在しない (六面体・角柱と逆)。三角形面上で N が
  面積座標に厳密に退化することが四面体と適合する理由 (u = ±1 面で確認できる)。
- 鉄損 (`bertotti`) は静磁場の |B| を波高値とみなす後処理。**重ね合わせが
  効かない**ので行列ではなく駆動ポート毎の値として持つ (`Pfemat` の対角)。
  3 項 (f¹B^α / f²B² / f^1.5B^1.5) は値が合っても分離が正しいとは限らないので、
  1 項ずつ残して f と I の指数を検査すること。

## 機能追加の規則

- 入力キー追加は `sol/input_data.c` に、既定値は「キー省略時に従来動作と
  完全一致」になるよう初期化する (後方互換)。
- **`input_data()` は非構造格子のとき途中で `return 0` する。** その後ろに
  書いた検査は構造格子にしか効かない。非構造格子専用の解析 (`E` / `A` / `P`)
  に効かせたい検査は、必ずその分岐の中に置くこと。実際に踏んだ: 「`direct = 1`
  は `F` / `A` で弾く」検査が `A` に対して一度も実行されておらず、検証も
  `F` しか見ていなかったので誰も気づかなかった。
- 新機能には data/sample/ の検証ケース (できれば解析解付き) と
  `data/sample/rlc_check.sh` への追加、CI スモーク (3 OS) を必ず付ける。
- `ofe_post` が `ofe.out` の新フィールドを無害に扱えることを確認する
  (フォーマットを変えたら先頭のマジック `OFEOUT01` も上げる)。
- CPU 実装にだけ追加した機能は README に対応状況を明記する。
- **形式を足すときは検証用ファイルを「本物のツール」に作らせる。** 自作の
  書き手と読み手が同じ誤解を共有していると、テストは通るのに実際のファイルで
  落ちる。Gmsh バイナリは gmsh 自身に変換させたものを置いてある。
  なお gmsh は変換のたびに節点を振り直すので、比較する ASCII 側も**同じ変換で
  出したもの**でないと丸めの順序が変わって一致しない。
- **MPI の並列化の粒度は「周波数掃引の点」だけ。** 各点の演算は直列と完全に
  同一なので、`mpirun -np N` の出力は直列と**バイト単位で一致**しなければ
  ならない (それが `rlc_check.sh` の [mpi] 節の恒等式。閉形式も許容誤差も
  要らない)。rank 0 だけがファイルを書き、他 rank のログは tmpfile に捨てる。
  **解けなかった点も pack (ierr つき) を必ず送ること** — ある rank だけが
  途中で止まると送受信の回数が食い違ってデッドロックする。掃引なし /
  `fieldout = 1` / 無効ビルドの mpirun 起動は黙って直列に落ちず入力エラー。
  mpi.h は `sol/mpiwrap.c` の外に出さない (MSVC ビルドを変えないため)。
- **外部ライブラリを使う機能は任意依存 (既定 OFF) にする。** 「素の CMake で
  3 OS ビルドできる」性質を既定のまま保つこと。無効ビルドではスタブに落とし、
  **その機能を要求されたら黙って無視せず入力エラーにする** (書いたつもりで
  ファイルが無いのが一番困る)。検証は「無効ビルドでは弾かれること」と
  「有効ビルドでは動くこと」の両方を見る (`hdf5` がその形)。

## 設定ファイル (.claude/)

詳細な規則は `.claude/rules/` に分けてあり、該当ファイルを編集するときだけ
自動で読み込まれます (`portability.md` / `numerics.md` / `validation.md`)。
`/build` `/verify` `/add-analysis` のスラッシュコマンドと、MSVC で落ちる書き方を
編集直後に検出するフックも入っています。詳しくは `.claude/README.md`。

## CI

`.github/workflows/ci.yml`: Linux / macOS (libomp) / Windows (MSVC + Ninja)。
既定のビルドに外部ライブラリ依存は無い (HDF5 と MPI だけ任意依存で、
`WITH_HDF5=ON` / `WITH_MPI=ON` の Linux ジョブが 1 つずつ別にある)。Windows の検証ステップは Git for Windows の bash で
`rlc_check.sh` を実行する。タグ `v*` push で Release にバイナリを添付。
