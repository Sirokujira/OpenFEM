...
- **角柱 (6 節点) は三角形 3 点則 x ζ 方向 2 点 Gauss の 6 点**。六面体と同じく
  ...
  依らなくなり、手抜きが検出できない)。
- **3 次元要素は「四面体 → 六面体 → 角柱」の連番で見る** (`elem3d_count` /
  `elem3d_kind` / `elem3d_nodes` / `elem3d_mat` / `elem3d_tag`)。種別を足すときは
  この 5 つと `assemble_elem3d` / `crs_alloc_elem3d` / `fieldout.c` / `hdf5out.c` を
  同時に直す。**並び順は変えないこと** — 純四面体格子で従来と完全一致する
  (既存サンプルの出力がバイト一致することで担保している)。
- **六面体と四面体は面を共有できない。** 四角形面のトレースは双 1 次、三角形面は
  1 次なので、接すると有限要素空間が H¹ に入らない (値は出るが誤答)。
  `elem_finish()` で入力エラーにする。角柱は両方の面を持つので四面体とも
  六面体とも適合し、間に挟めばつながる。**ピラミッドはこの遷移を 1 要素で
  行うための要素**で、対応すれば六面体 → 四面体が直接つながる。
