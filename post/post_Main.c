...
static int readout(const char *fname, result_t *res)
{
	...
	if (!ierr) {
		...
		if ((res->np < 1) || (nn > 1000000)) {
			...
			// 等価回路に使う L / 直列 R の優先順位:
			//   L : 渦電流 L(f) > TEM (外部) > 静磁場 L_dc
			//   R : 渦電流 R(f) > DC 直列抵抗 Rs
			res->lind = (res->haveF ? res->lf : (res->haveL ? res->l : res->m));
			res->rser = (res->haveF ? res->rf : res->s);
		}
	}
...
static void write_spice(const result_t *res)
{
	...
	if (pul && (res->haveL || res->haveM || res->haveF)) {
		fprintf(fp, "* L = %s\n",
			(res->haveF ? "eddy current L(f) (incl. skin effect)" :
			 res->haveL ? "TEM (external only)" : "magnetostatic (incl. internal)"));
	}
	if (res->haveF) {
		fprintf(fp, "* series R is the eddy current R(f) at %.6e [Hz]\n", res->freq);
	}
	else if (res->haveS) {
		fprintf(fp, "* series R is the DC conductor resistance (no skin effect)\n");
	}
	...
	if (!pul) {
		...
		for (int s = 0; s < ns; s++) {
			...
			for (int i = 0; i < np; i++) {
				...
				// 直列 R -> 直列 L の順に並べる
				char nmid[64];
				if ((res->haveS || res->haveF) && (res->rser[(i * np) + i] > 0)) {
				...
				if ((res->haveL || res->haveM || res->haveF) && (res->lind[(i * np) + i] > 0)) {
			...
			for (int i = 0; i < np; i++) {
				for (int j = i + 1; j < np; j++) {
					...
					if (res->haveL || res->haveM || res->haveF) {
