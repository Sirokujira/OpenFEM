...
typedef struct {
	int32_t np;
	int32_t haveC, haveL, haveR, nsection;
	int32_t tline;
	double  length, volt;
	char    title[256];
	double  *c, *l, *g, *r;
...
static int readout(const char *fname, result_t *res)
{
	...
	if (!ierr) {
		if ((fread(&res->np, sizeof(int32_t), 1, fp) != 1) ||
		    (fread(flag, sizeof(int32_t), 4, fp) != 4) ||
		    (fread(&res->tline, sizeof(int32_t), 1, fp) != 1) ||
		    (fread(dval, sizeof(double), 2, fp) != 2) ||
...
static void write_csv(const result_t *res)
{
	...
	if (res->haveC && res->haveL && (np == 1) && (res->c[0] > 0) && (res->l[0] > 0)) {
		const double c = 2.99792458e8;
		fprintf(fp, "Z0 [ohm],%.8e\n", sqrt(res->l[0] / res->c[0]));
		...
		for (int s = 0; s < ns; s++) {
			...
			for (int i = 0; i < np; i++) {
				for (int j = i + 1; j < np; j++) {
					...
					if (res->haveL) {
						const double lii = res->l[(i * np) + i];
						const double ljj = res->l[(j * np) + j];
						const double lij = res->l[(i * np) + j];
						if ((lii > 0) && (ljj > 0)) {
